...
class hog_descriptor():
    ...
    def extract(self):
        ...
        gradient_magnitude = abs(gradient_magnitude)#幅度取绝对值
        '''
        ...
        print(np.asarray(hog_vector).shape)
        print(np.asarray(hog_vector))
    ...
    def global_gradient(self):
        ...
        分别计算图像沿x轴和y轴的梯度
        '''
        # 利用Sobel方法可以进行sobel边缘检测 # img表示源图像，即进行边缘检测的图像 # cv2.CV_64F表示64位浮点数即64float。
        #  这里不使用numpy.float64，因为可能会发生溢出现象。用cv的数据则会自动
        #  第三和第四个参数分别是对X和Y方向的导数（即dx,dy），这里1表示对X求偏导，0表示不对Y求导。其中，X还可以求2次导。
        #  注意：对X求导就是检测X方向上是否有边缘。 # 第五个参数ksize是指核的大小。
        gradient_values_x = cv2.Sobel(self.img, cv2.CV_64F, 1, 0, ksize=5)#对x求导
        gradient_values_y = cv2.Sobel(self.img, cv2.CV_64F, 0, 1, ksize=5)#对y求导
        # 计算梯度幅值 这个计算的是0.5*gradient_values_x + 0.5*gradient_values_y
        # gradient_magnitude = cv2.addWeighted(gradient_values_x, 0.5, gradient_values_y, 0.5, 0)
        # 计算梯度方向
        # gradient_angle = cv2.phase(gradient_values_x, gradient_values_y, angleInDegrees=True)
        #print(gradient_values_x,gradient_values_y)
...
if __name__ == '__main__':
    # 加载图像
    img = cv2.imread('data/person.png')
    # width = 64
    # height = 128
